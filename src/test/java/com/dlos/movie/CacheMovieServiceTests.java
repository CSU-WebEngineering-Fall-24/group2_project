...
public class CacheMovieServiceTests {
	...
	private final Movie hercules = new Movie() {
		{
			...
			setPoster(
					"https://m.media-amazon.com/images/M/MV5BMmUyZGY1ZjItMWYwZS00MjJjLTg1YzAtY2Q5OTA4MmFhYzFjXkEyXkFqcGc@._V1_SX300.jpg");
			setRatings(new Rating[] {
					new Rating() {
						{
							setSource("Internet Movie Database");
	...
	public void MovieIdCacheTest() {
		...
		Movie movie = movieCache.getMovie(terminator.getImdbID());
		Assertions.assertEquals(1, movieCache.getMovieIdsCacheCount());
		Assertions.assertEquals(0, movieCache.getMovieIdCacheHitCount());

		movie = movieCache.getMovie(hercules.getImdbID());
		movie = movieCache.getMovie(terminator.getImdbID());
		movie = movieCache.getMovie(hercules.getImdbID());
		...
		movie = movieCache.getMovie(terminator.getImdbID() + "_2");
		movie = movieCache.getMovie(hercules.getImdbID() + "_2");
